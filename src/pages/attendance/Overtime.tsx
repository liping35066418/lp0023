...
export default function OvertimeManagement() {
  ...
  useEffect(() => {
    fetchRecords();
  }, [page, keyword, status, overtimeType]);
  ...
  const fetchRecords = async () => {
    try {
      ...
      if (overtimeType) params.append('overtimeType', overtimeType);

      const res = await fetch(`/api/attendance/overtime?${params}`);
  ...
  const getMockData = (): OvertimeRecord[] => [
    { id: 1, employee_id: 6, employee_name: '孙八', employee_no: 'EMP006', department_name: '前端组', overtime_date: '2024-07-02', start_time: '18:30', end_time: '21:30', hours: 3, overtime_type: '工作日加班', reason: '版本上线，加班处理紧急bug', status: 'approved', approver: '赵六', created_at: '2024-07-02' },
    { id: 2, employee_id: 7, employee_name: '周九', employee_no: 'EMP007', department_name: '后端组', overtime_date: '2024-07-05', start_time: '18:00', end_time: '22:00', hours: 4, overtime_type: '工作日加班', reason: '新功能开发', status: 'approved', approver: '赵六', created_at: '2024-07-05' },
    { id: 3, employee_id: 4, employee_name: '赵六', employee_no: 'EMP004', department_name: '技术部', overtime_date: '2024-07-06', start_time: '09:00', end_time: '18:00', hours: 8, overtime_type: '周末加班', reason: '项目赶进度', status: 'pending', created_at: '2024-07-05' },
    { id: 4, employee_id: 5, employee_name: '钱七', employee_no: 'EMP005', department_name: '市场部', overtime_date: '2024-07-07', start_time: '10:00', end_time: '16:00', hours: 6, overtime_type: '周末加班', reason: '活动筹备', status: 'pending', created_at: '2024-07-06' },
    { id: 5, employee_id: 8, employee_name: '吴十', employee_no: 'EMP008', department_name: '测试组', overtime_date: '2024-07-03', start_time: '18:00', end_time: '20:30', hours: 2.5, overtime_type: '工作日加班', reason: '回归测试', status: 'rejected', approver: '赵六', remark: '建议在工作时间内完成', created_at: '2024-07-03' },
    { id: 6, employee_id: 2, employee_name: '李四', employee_no: 'EMP002', department_name: '人力资源部', overtime_date: '2024-06-30', start_time: '18:00', end_time: '20:00', hours: 2, overtime_type: '工作日加班', reason: '月底薪资核算', status: 'approved', approver: '张三', created_at: '2024-06-30' },
  ];
  ...
  const getOvertimeTypeColor = (type: string) => {
    const map: Record<string, string> = {
      '工作日加班': 'bg-blue-100 text-blue-700',
      '周末加班': 'bg-orange-100 text-orange-700',
      '节假日加班': 'bg-red-100 text-red-700',
    };
  ...
  const handleSubmit = async () => {
    if (!formData.employee_id || !formData.overtime_date || !formData.start_time || !formData.end_time) {
      alert('请填写必填项');
      return;
    }
    try {
      const res = await fetch('/api/attendance/overtime', {
        ...
        body: JSON.stringify({
          ...
          overtimeDate: formData.overtime_date,
          startTime: formData.start_time,
          endTime: formData.end_time,
          overtimeType: formData.overtime_type,
  ...
  const stats = [
    { label: '本月加班总时长', value: '128h', color: 'text-orange-600' },
    { label: '待审批', value: '4', color: 'text-yellow-600' },
    { label: '已通过', value: '22', color: 'text-green-600' },
    { label: '人均加班', value: '12.8h', color: 'text-blue-600' },
  ];
    ...
    <div className="space-y-6">
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-center text-sm font-medium text-gray-600">加班类型</th>
                ...
                <th className="px-4 py-3 text-center text-sm font-medium text-gray-600">时长</th>
                ...
                return (
                  <tr key={record.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${getOvertimeTypeColor(record.overtime_type)}`}>
                        {record.overtime_type}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center text-sm text-gray-700">{record.overtime_date}</td>
                    <td className="px-4 py-3 text-center text-sm text-gray-700">
                      {record.start_time} - {record.end_time}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className="text-sm font-medium text-orange-600">{record.hours}h</span>
                    </td>
                    <td className="px-4 py-3 text-center">
          ...
          <div className="bg-white rounded-xl w-4/5 max-w-lg max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-6">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm text-gray-600 mb-1">加班类型</label>
                  <select
                ...
                <div>
                  <label className="block text-sm text-gray-600 mb-1">加班日期 <span className="text-red-500">*</span></label>
                  <input
                    type="date"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">结束时间 <span className="text-red-500">*</span></label>
                    <input
                      type="time"
                ...
                <div>
                  <label className="block text-sm text-gray-600 mb-1">加班原因</label>
                  <textarea
      ...
      {showDetail && selectedRecord && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl w-4/5 max-w-lg max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-800">加班详情</h3>
            ...
            <div className="flex-1 overflow-auto p-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center">
                  <span className="w-20 text-sm text-gray-500">加班类型</span>
                  <span className={`px-2 py-0.5 rounded text-sm font-medium ${getOvertimeTypeColor(selectedRecord.overtime_type)}`}>
                    {selectedRecord.overtime_type}
                  </span>
                </div>
                <div className="flex items-center">
                  <span className="w-20 text-sm text-gray-500">加班日期</span>
                  <span className="text-gray-800">{selectedRecord.overtime_date}</span>
                </div>
                <div className="flex items-center">
                  <span className="w-20 text-sm text-gray-500">时间段</span>
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-orange-600 font-medium">{selectedRecord.hours} 小时</span>
                </div>
                <div>
